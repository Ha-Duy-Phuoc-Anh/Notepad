...
LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_COMMAND: // Menu and control commands
		...
		case ID_EDIT_UNDO:
			...
			break;
		case ID_EDIT_CUT:
			SendMessageW(hEditor, WM_CUT, 0, 0);
			break;
		case ID_EDIT_COPY:
			SendMessageW(hEditor, WM_COPY, 0, 0);
			break;
		case ID_EDIT_PASTE:
			SendMessageW(hEditor, WM_PASTE, 0, 0);
			break;
		case ID_EDIT_DELETE:
			SendMessageW(hEditor, WM_CLEAR, 0, 0);
			break;
		case ID_EDIT_SEARCH_BROWSER: {
			// Get text range
			DWORD selectRange = SendMessageW(hEditor, EM_GETSEL, 0, 0);
			int startChar = LOWORD(selectRange);
			int endChar = HIWORD(selectRange);

			// Retrieve selected text
			int textLength = GetWindowTextLengthW(hEditor);

			// Extract selected portion
			LPWSTR selectedText = (LPWSTR)malloc((endChar - startChar + 1) * sizeof(TCHAR));
			SendMessageW(hEditor, EM_GETSELTEXT, 0, (LPARAM)selectedText);
			free(selectedText);
	...
	case WM_INITMENUPOPUP: {
		UINT stateUndo = (SendMessageW(hEditor, EM_CANUNDO, 0, 0)) ? MF_ENABLED : MF_GRAYED;
		EnableMenuItem(hMenu, ID_EDIT_UNDO, stateUndo);

		UINT stateCut = (SendMessageW(hEditor, EM_GETSEL, 0, 0) & 0xFFFF) != (SendMessageW(hEditor, EM_GETSEL, 0, 0) >> 16) ? MF_ENABLED : MF_GRAYED;
		EnableMenuItem(hMenu, ID_EDIT_CUT, stateCut);

		UINT stateCopy = (SendMessageW(hEditor, EM_GETSEL, 0, 0) & 0xFFFF) != (SendMessageW(hEditor, EM_GETSEL, 0, 0) >> 16) ? MF_ENABLED : MF_GRAYED;
		EnableMenuItem(hMenu, ID_EDIT_COPY, stateCopy);

		UINT statePaste = (OpenClipboard(hwnd) && IsClipboardFormatAvailable(CF_TEXT) ? MF_ENABLED : MF_GRAYED);
		EnableMenuItem(hMenu, ID_EDIT_PASTE, statePaste);

		UINT stateDelete = (SendMessageW(hEditor, EM_GETSEL, 0, 0) & 0xFFFF) != (SendMessageW(hEditor, EM_GETSEL, 0, 0) >> 16) ? MF_ENABLED : MF_GRAYED;
		EnableMenuItem(hMenu, ID_EDIT_DELETE, stateDelete);
	}
...
void AddMenu(HWND hwnd) {
	...
	AppendMenuW(hEditMenu, MF_STRING, ID_EDIT_DELETE, L"Delete\tDel");
	AppendMenuW(hEditMenu, MF_SEPARATOR, 0, NULL);
	AppendMenuW(hEditMenu, MF_STRING, ID_EDIT_SEARCH_BROWSER, L"Search with Bing...\tCtrl+F");
